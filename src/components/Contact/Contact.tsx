          ...
          <ul className="grid grid-cols-1 gap-px border border-accent bg-accent sm:grid-cols-2 lg:grid-cols-4">
            {contacts.map((contact, index) => (
              <Reveal key={contact.id} as="li" delay={index * 70} variant="scale-in" duration={650}>
                <a
                  href={contact.href}
                  className="flex min-h-11 flex-col justify-center gap-1 bg-bg-card px-4 py-4 transition-[color,transform] duration-200 active:scale-[0.99] active:text-accent motion-safe:hover:text-accent"
